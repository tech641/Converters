...
import "./Invoices.css"; // reuse your page styles
...
function ImporterBank() {
  const title = "Upload Invoices";
  const addLabel = "Add Invocies";
  const importLabel = "Import";
  const downloadLabel = "Invoice Excel";
  const accept = ".pdf,image/*";
  const resultsTitle = "Invoice Results";
  const downloadFileName = "invoice.xlsx";
    ...
    try {
      ...
      navigate("/converts/invoices/tableresult", {
        state: {
          ...
          columns,
          rows,
          downloadLabel: "Download Invoices",
          downloadFileName: "invoice.csv",
        },
      });
    } finally {
      clearInterval(timer);
