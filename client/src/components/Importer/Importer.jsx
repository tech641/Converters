...
export default function Importer({
  ...
  tableColumns = [],
  tableRows = [],
  onImport, // optional async callback(files) -> {columns, rows, downloadUrl}
}) {
  const fileInputRef = useRef(null);

  const [phase, setPhase] = useState("files");            // "files" | "results"
  const [queue, setQueue] = useState([]);                 // uploading with progress
  const [files, setFiles] = useState([]);                 // finished files
  const [selectedId, setSelectedId] = useState(null);

  const [importing, setImporting] = useState(false);      // overlay flag
  const [importProgress, setImportProgress] = useState(0);// 0..100 (fake or real)

  const [resColumns, setResColumns] = useState(tableColumns);
  const [resRows, setResRows] = useState(tableRows);
  ...
  function readWithProgress(file) {
    ...
    reader.onload = () => {
      ...
      setFiles(prev => {
        const next = [...prev, { id, name: file.name, size: file.size, url, type }];
        if (!selectedId) setSelectedId(id); // first finished becomes selected
        return next;
      });
    };
    ...
    // We only use this to get progress events:
  ...
  // ---- Import flow with circular loader ----
  async function handleImport() {
    ...
    let timer;
    try {
      // If you provide onImport, run it in parallel while we animate progress.
      // You can also report real progress by calling setImportProgress from your callback
      // (just pass it down).
      timer = animateProgress(setImportProgress);

      if (onImport) {
        const res = await onImport(files);
        if (res?.columns) setResColumns(res.columns);
        if (res?.rows) setResRows(res.rows);
        if (res?.downloadUrl) setDownloadUrl(res.downloadUrl);
      }

      // ensure we end at 100% then show results
      setImportProgress(100);
      setTimeout(() => setPhase("results"), 200);
    } finally {
      clearInterval(timer);
      setTimeout(() => setImporting(false), 220); // close overlay smoothly
    }
  }

  // simple animated progress (fake) – 0→90% quickly, hold, then complete
  ...
  return (
    <section className="imp2">
        ...
        <div className="imp2-grid">
          ...
          <div className="pane-left">
            <div className="upload-card">
              ...
              <p className="upload-sub">Choose PDF or image files and import.</p>
              <div className="upload-actions">
                <button className="btn soft" onClick={openPicker} aria-label={addLabel}>
                ...
                <ul className="queue-list">
                  {queue.map(q => (
                    <li key={q.id} className="queue-item">
                      ...
                      <div className="progress"><div className="bar" style={{ width: `${q.percent}%` }} /></div>
                    </li>
                  ))}
                </ul>
              </div>
                ...
                <ul className="file-list">
                  {files.map(f => (
                    <li key={f.id} className={`file-item ${selectedId === f.id ? "active" : ""}`}>
                      <button className="file-main" onClick={() => setSelectedId(f.id)} title={f.name}>
                        <span className="icon">{f.type === "image" ? <ImageIcon size={16}/> : <FileText size={16}/>}</span>
                        <span className="name">{f.name}</span>
                      </button>
                      <button className="icon-btn danger" onClick={() => removeFile(f.id)} aria-label="Delete">
                        <Trash2 size={16}/>
                      </button>
                    </li>
                  ))}
      ...
      {/* ---- Import overlay (circular loader) ---- */}
      {importing && (
            ...
            <div className="ring">
              <svg viewBox="0 0 44 44">
                ...
                <circle
                  ...
                  r="20"
                  style={{
                    strokeDasharray: 125.6, // 2πr with r=20
